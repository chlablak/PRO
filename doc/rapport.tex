...
\begin{document}
	...
	\section{Interpréteur} % Champion
		...
		\subsection{Normalisation du langage} 
			...
			\subsubsection{Modifications}
				...
				\textcolor{red}{AJOUTER DATE}\\
				\textcolor{red}{nouvelles modifications depuis l'impl. du parseur}\\
				\textcolor{red}{oublis des += et -= dans la grammaire}\\
				\textcolor{red}{oubli de l'id dans connection}\\
				\textcolor{red}{sign->negate() -> impossible d'ecrire v=(1::-2) (en l'état)}
	...
	\label{sec:graphes}
		...
		\subsection{Architecture}
			...
			La création de graphe peut maintenant s'effectuer en utilisant la classe Graph. Voici un exemple d'utilisation suivant le type de graphe à créer.
			\begin{lstlisting}
						public static void main() {
							// creer quelques sommets
							Vertex v1, v2, v3;
							Edge e1(v1, v2);
							Edge e2(v1, v3);
							
							// Creation d'un graphe non oriente
							IGraph *myGraph = new Graph({v1, v2, v3}, {e1, e2});
							
							DiEdge de1(v1, v2);
							DiEdge de2(v2, v3);
							
							// Creation d'un graphe oriente
							IGraph *myDiGraph = new DiGraph({v1, v2, v3}, {de1, de2});
							
							// Creation d'un graphe a flot
							IGraph *myFlowGraph = new FlowGraph({v1, v2, v3}, {de1, de2});
		...
		\subsection{Algorithmes}
			...
			% TODO corriger UML et code d'exemple
			\begin{figure}[H]
				\centering
	...
	\section{Représentation graphique des graphes}
		\subsection{Widget de visualisation}
			\subsubsection{Fonctionnement de QT}
				QT possède un système de dessin complexe, le \textit{Graphics View Framework}. Cet outil est basé sur le principe suivante : trois type d'éléments se partagent différentes responsabilités. \\
			...
			\subsubsection{Diagramme de classe}
				...
				Puis les éléments graphiques \textbf{VertexItem} et \textbf{EdgeItem} (ainsi que ses sous-classes \textbf{DiEdgeItem} et \textbf{FlowEdgeItem}) permette respectivement de dessiner les sommets et les arcs/arêtes. Ces classes ne sont pas directement de type QGraphcsItem, mais de QGraphicsItemGroup qui est en faite un conteneur de plusieurs QGraphcsItem. En effet, une sommet est composé d'un cercle (réellement d'une ellipse) d'un texte pour l'identifiant affiché et d'une autre texte pour son label. Les arêtes (EdgeItem) qui sont des simple lignes avec un label, les arcs (DiEdge) qui ajoutent une flèche pour la direction et les flux (FlowEdge) qui ajout également une flèche, mais aussi les capacité. Chacun des éléments graphiques ajoute et gère tous les éléments graphiques dont il a besoin. \\
		...
		\subsection{Fonctions incluses}
			...
			\begin{figure}[H]
				...
				\begin{tabular}{lll}
					...
					\texttt{Graph load(String file)} & Charge le graphe contenu dans le fichier \texttt{file} & \texttt{built-in}\\
					\texttt{Boolean draw(Graph g)} & Dessine le graphe \texttt{g} & \texttt{GUI}\\
				\end{tabular}
			\end{figure}
