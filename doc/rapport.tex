...
\begin{document}
	...
	\label{sec:graphes}
		...
		\subsection{Algorithmes}
			...
			\subsubsection{Liste}
			Dans le tableau ci-dessous figurent les algorithmes traités par l'application. Pour la colonne \textit{Complexité}, $n$ représente le nombre de sommet, et $m$ le nombre d'arcs/arêtes d'un graphe.
			\begin{figure}[H]
				\centering
				\begin{tabular}{lL{10cm}l}
					\textbf{Nom} & \textbf{Description} & \textbf{Complexité}\\
					\hline
					BFS & Breadth First Search : effectue une exploration en largeur d'un graphe depuis un sommet source et retourne l'arborescence du parcours. & $O(n+m)$\\
					\hline
					DFS & Deapth First Search : effectue une exploration en profondeur d'un graphe depuis un sommet source et retourne l'arborescence du parcours. & $O(n+m)$\\
					\hline
					Composante connexe & Calcul les composantes connexes d'un graphe et retourne un tableau dont l'index représente le sommet et la valeur la composante à laquelle appartient le sommet. & $O(n+m)$\\
					\hline
					Kruskal & Implémenté avec UnionFind, recherche de l'arbre recouvrant de poids min. & $O(m * log(n))$\\
					\hline
					Prim & Version empressée de Prim, recherche de l'arbre recouvrant de poids min. & $O(m * log(n))$\\
				\end{tabular}
			\end{figure}
