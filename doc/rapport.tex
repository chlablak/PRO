...
\lstset{
	...
	gobble=10,
	frame=single,
	otherkeywords={std,string,vector,map,pair,size\_t,function,remove\_const,remove\_reference}
}
			...
			\subsubsection{Interfaçage}
				...
				\begin{lstlisting}
					...
					int main()
					{
						...
						FuncTable func(var);
						func.names["add"] = new Func<int, int, int>(&add);
						func.names["inv"] = new Func<int, int>(&inv);
						func.names["len"] = new Func<int, string const&>(&len);
						func.names["hw"] = new Func<string>(&hw);
				...
				Le coeur de la solution est la classe de base abstraite \texttt{FuncBase} dont hérite la classe \textit{templatisée} \texttt{Func} qui effectue l'appel effectif à la fonction interfacée.\\
				
				On remarque que cela fonctionne bien, et que côté utilisateur l'ajout d'une nouvelle fonction est très agréable (un peu moins que ce qui était attendu, mais ça reste très correct).
